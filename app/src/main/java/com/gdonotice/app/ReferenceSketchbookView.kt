...
class ReferenceSketchbookView(context: Context) : View(context) {
    ...
    /** Fit the 390×800 reference artboard inside any phone viewport without clipping controls. */
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        val px = (event.x - leftInset()) / s()
        val py = (event.y - topInset()) / s()
        page = when (page) {
            Page.HOME -> when {
                px > 325f && py < 90f -> Page.ACCOUNT
                py in 440f..550f && px < 130f -> Page.DRAW
                py in 440f..550f && px in 130f..260f -> Page.ADD
                py in 440f..550f && px > 260f -> Page.ADD
                py > 700f && px > 190f -> Page.LIBRARY
                else -> Page.HOME
            }
            Page.ADD -> when {
                py > 470f && py < 600f && px < 205f -> Page.SIZE
                py > 470f && py < 600f && px >= 205f -> Page.LIBRARY
                py < 440f && px > 320f -> Page.HOME
                else -> Page.ADD
            }
            Page.SIZE -> {
                CanvasSizeInteraction.presetIndex(px, py)?.let { selectedCanvasIndex = it }
                if (CanvasSizeInteraction.shouldCreate(py)) Page.DRAW else Page.SIZE
            }
            Page.LIBRARY -> when {
                py > 650f && px > 300f -> Page.ADD
                py > 700f && px < 190f -> Page.HOME
                py in 90f..700f -> Page.DRAW
                else -> Page.LIBRARY
            }
            Page.DRAW -> if (py < 90f && px < 70f) Page.LIBRARY else Page.DRAW
            Page.ACCOUNT -> if (py < 90f && px < 70f) Page.HOME else Page.ACCOUNT
        }
        invalidate()
        return true
    }
