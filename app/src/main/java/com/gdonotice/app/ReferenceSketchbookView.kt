...
class ReferenceSketchbookView(context: Context) : View(context) {
    ...
    private fun size(canvas: Canvas) { header(canvas,"New sketchbook"); val labels=listOf("A4","A5","B5","1:1","4:3","9:16"); labels.forEachIndexed { i,label -> val col=i%3; val row=i/3; val l=38+col*110; val t=105+row*172; val h=if(i<3) 99f else if(i==5) 115f else 72f; rect(canvas,l,t,l+63,t+h,2f,Color.TRANSPARENT, if(i==1) blue else 0xFF77726B.toInt(), if(i==1)2f else 1.3f); if(i<3) { line(canvas,l+45,t,l+45,t+18,ink); line(canvas,l+45,t+18,l+63,t+18,ink) }; text(canvas,label,14f,l+31,t+h+26,ink,false,true); if(i==1){ paint.style=Paint.Style.FILL; paint.color=blue; canvas.drawCircle(x(l+51),y(t+11),x(9),paint); text(canvas,"✓",11f,l+51,t+15,Color.WHITE,false,true) } }; text(canvas,"⚙  Custom size",13f,22f,674f,blue); rect(canvas,271,647,368,698,28f,blue); text(canvas,"Create",16f,319f,679f,Color.WHITE,false,true) }
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean { if(event.action != MotionEvent.ACTION_UP) return true; val px=event.x/s(); val py=event.y/s(); page=when(page){Page.HOME-> when { py>440&&py<550&&px in 150f..240f->Page.ADD; px>325&&py<90->Page.ACCOUNT; py>720&&px>190->Page.LIBRARY; else->Page.HOME }; Page.ADD->if(py>470&&py<580&&px<210) Page.SIZE else Page.ADD; Page.SIZE->if(py>620) Page.LIBRARY else Page.SIZE; Page.LIBRARY->if(py in 95f..300f&&px<200) Page.DRAW else Page.LIBRARY; Page.DRAW->if(py<85&&px<65) Page.LIBRARY else Page.DRAW; Page.ACCOUNT->if(py<85&&px<65)Page.HOME else Page.ACCOUNT}; invalidate(); return true }
}
