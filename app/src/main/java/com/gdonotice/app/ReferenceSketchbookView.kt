...
class ReferenceSketchbookView(context: Context) : View(context) {
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean { if(event.action != MotionEvent.ACTION_UP) return true; val px=(event.x-leftInset())/s(); val py=(event.y-topInset())/s(); page=when(page){Page.HOME-> when { py>440&&py<550&&px in 150f..240f->Page.ADD; px>325&&py<90->Page.ACCOUNT; py>710&&px>190->Page.LIBRARY; else->Page.HOME }; Page.ADD->if(py>470&&py<580&&px<210) Page.SIZE else Page.ADD; Page.SIZE-> { CanvasSizeInteraction.presetIndex(px, py)?.let { selectedCanvasIndex = it }; if(CanvasSizeInteraction.shouldCreate(py)) Page.LIBRARY else Page.SIZE }; Page.LIBRARY->if(py in 95f..300f&&px<200) Page.DRAW else Page.LIBRARY; Page.DRAW->if(py<85&&px<65) Page.LIBRARY else Page.DRAW; Page.ACCOUNT->if(py<85&&px<65)Page.HOME else Page.ACCOUNT}; invalidate(); return true }
}
